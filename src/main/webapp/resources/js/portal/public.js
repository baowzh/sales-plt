...
if (Browser.isFF) {
	...
	window.constructor.prototype.showModalDialog = function(url, arg,
			parameters) {
		parameters = "width="
				+ arg.width
				+ ",innerHeight="
				+ (parseInt(arg.height) + 30)
				+ ",toolbar=no,menubar=no,scrollbars=no,location=no,resizable=no,alwaysRaised=yes,depended=yes";
	...
	if (window.Event) {
		...
		Event.prototype.__defineGetter__("srcElement", function() {
			...
			while (node.nodeType != undefined && node.nodeType != 1)
...
function getParameter(param) {
	...
	if (end == -1)
		return decodeURIComponent(paramstr.substring(start));
...
function checkedAll(boxName, ischecked) {
	var boxList = getElements(boxName);
	for ( var i = 0; i < boxList.length; i++) {
		if (boxList[i].disabled || boxList[i].getAttribute("flag") == "temp")
			continue;
...
function getDataByStr(obj, group, istrim) {
	...
	for ( var i = 0; i < elements.length; i++) {
		...
		if (name == "listener" || name == "service" || name == "sp"
				|| name == "Form0")
			continue;
	...
	for ( var i = 0; i < forms.length; i++) {
		...
		for ( var j = 0; j < elements.length; j++) {
			...
			if (element.tagName == "INPUT"
					&& (element.type.toUpperCase() == "SUBMIT" || element.type
							.toUpperCase() == "BUTTON")) {
	...
	if (window.event.keyCode == 13) {
		...
		if (srcelement != null) {
			if (srcelement.tagName == "INPUT"
					&& srcelement.type.toUpperCase() == "TEXT"
					&& !srcelement.disabled && !srcelement.readOnly) {
...
function addObjEventListener(obj, eventName, eventHandler) {
	if (eval(obj + ".addEventListener")) {
		eval(obj + ".addEventListener('" + eventName + "', " + eventHandler
				+ ", false);");
...
function removeObjEventListener(obj, eventName, eventHandler) {
	if (eval(obj + ".removeEventListener")) {
		eval(obj + ".removeEventListener('" + eventName + "', " + eventHandler
				+ ", false);");
...
function getChildElement(obj, tag, sign) {
	...
	for ( var i = 0; i < nodes.length; i++) {
		if (nodes[i].tagName == tag.toUpperCase() && nodes[i].sign == sign)
			return nodes[i];
...
function getElementBySrc() {
	if (Browser.isFF) {
		return window.event == null ? null : window.event.target;
	} else {
...
function openDialog(url, width, height, arg) {
	var returnValue = window.showModalDialog(url, arg, "dialogWidth: " + width
	...
	return returnValue;
	// return window.showModalDialog(url, arg, "dialogWidth: " + width + "px;
	// dialogHeight: " + getDialogHeight(height) + "px; resizable: no; help: no;
	// status: no; scroll: no;");
}
/** popup dialog */
function popupDialog(page, listener, params, title, width, height, subsyscode,
		subsysaddr) {
	if (title == null)
		title = "弹出窗口";
	if (width == null)
	...
	var returnValue = openDialog(getSysAddr(getContextName()
			+ "?service=page/component.Agent&random=" + getRandomParam()),
			width, height, obj);
	if (returnValue != null && !Browser.isFF) {
		var pageName = returnValue.$pageName;
		if (pageName != null) {
			redirectTo(pageName, returnValue.$listener,
					returnValue.$parameters, returnValue.$target);
		}
	}
	return returnValue;
}
/** popup page */
function popupPage(page, listener, params, title, width, height, callback) {
	...
	hiddenInput.name = 'hiddenCallBack';
	hiddenInput.id = 'hiddenCallBack';
	if (document.forms != null && document.forms[0] != null) {
		document.forms[0].appendChild(hiddenInput);
		document.getElementById("hiddenCallBack").value = callback;
	}

	var returnValue = popupDialog(page, listener, params, title, width, height,
			imgobj.getAttribute("subsyscode"), imgobj
					.getAttribute("subsysaddr"));
	if (returnValue != null && !Browser.isFF) {
...
function setReturnObj(obj) {
	...
	if (Browser.isFF) {
		...
		if (parent.window.document.getElementById("callback") != null) {
			callback = parent.window.document.getElementById("callback").value;
		}
		if (window.document.getElementById("callback") != null) {
			callback = window.document.getElementById("callback").value;
		}
	} else {
		window.returnValue = obj;
	}
	if (Browser.isFF && callback != null && callback != ''
			&& callback != undefined) {
		beforeDialogClose(callback);
	} else if (Browser.isFF && callback == null) {
		// document.getElementById("hiddenCallBack").value
		if (top.window.opener.document.getElementById("hiddenCallBack")) {
			var hiddenCallBack = top.window.opener.document
					.getElementById("hiddenCallBack").value;
			beforeDialogClose(hiddenCallBack);
		}
	}
	top.close();
}
/**
 * set return value example 1: setReturnValue('key', 'value'); example 2:
 * setReturnValue('key', 'value', ['key1', 'key2'], ['value1', 'value2'])
 */
...
function popupUploadDialog(objname) {
	...
	if (editobj.value != "" && fileTotal != null
			&& parseInt(fileTotal) == editobj.value.split("/").length) {
		alert("上传文件数不能超过" + fileTotal + "个!");
		return false;
	}
	var callback = "setattcheUrl(\'" + "UPLOAD_DIV_" + objname + "\',\'"
			+ objname + "\')";
	var hiddenInput = document.createElement("input");
	hiddenInput.type = 'hidden';
	hiddenInput.name = 'hiddenCallBack';
	...
	var retobj = openDialog("component/popups/editor/editor_agent.html",
			obj.width, obj.height, obj);

	if (retobj != null && !Browser.isFF) {
		var filehref = record == "true" && reserved == "false" ? "attach?action=delete&file_id="
				+ retobj.fileId
				: "javascript:void(0)";
		var fileobj = document.createElement("SPAN");
		fileobj.innerHTML += (ctlobj.innerHTML == "" ? "" : "<br/>")
				+ retobj.fileUrl;
		fileobj.innerHTML += "<a href='"
				+ filehref
				+ "' target='printframe' onclick='if (!window.confirm(\"确定删除文件么？\")) return false;hidden(parentElement, true);deleteUploadFile(\""
				+ objname
				+ "\", \""
				+ retobj.fileId
				+ "\");'><img src='component/images/dellzt.gif' alt='删除' width='16' height='15' border='0'/></a>";
		ctlobj.appendChild(fileobj);

		editobj.value += (editobj.value == "" ? "" : "/") + retobj.fileId + ":"
				+ retobj.fileName;
	}
...
function setUploadFieldContent(objname, isdelete) {
	...
	for ( var i = 0; i < attachs.length; i++) {
		...
		var fileobj = document.createElement("SPAN");
		fileobj.innerHTML += (ctlobj.innerHTML == "" ? "" : "<br/>")
				+ getFileImage(fileName) + "<a href='attach?file_id=" + fileId
				+ "'>" + fileName + "</a>";
		if (isdelete != null && isdelete)
			fileobj.innerHTML += "<a href='attach?action=delete&file_id="
					+ fileId
					+ "' target='printframe' onclick='if (!window.confirm(\"确定删除文件么？\\n注：删除后需要提交[更新|修改]操作才能生效！\")) return false;hidden(parentElement, true);deleteUploadFile(\""
					+ objname
					+ "\", \""
					+ fileId
					+ "\");'><img src='component/images/dellzt.gif' alt='删除' width='16' height='15' border='0'/></a>";
...
function getSysAddr(url, subsyscode, subsysaddr) {
	...
	var subSysCode = subsyscode != null ? subsyscode : pagevisit
			.getAttribute("subSysCode");
	var epachyId = pagevisit.getAttribute("loginEpachyId");

	if (staffId != null && addr.indexOf("&%73taffId=") == -1)
		addr += "&staffId=" + staffId;
	if (departId != null && addr.indexOf("&%64epartId") == -1)
		addr += "&departId=" + departId;
	if (subSysCode != null && addr.indexOf("&%73ubSysCode=") == -1)
		addr += "&subSysCode=" + subSysCode;
	if (epachyId != null && addr.indexOf("&%65parchyCode=") == -1)
		addr += "&eparchyCode=" + epachyId;
...
function getContextName() {
	...
	/*
	 * if (obj != null && obj.getAttribute("subsys") != null &&
	 * obj.getAttribute("subsys") != "") { return contextName; }
	 */
...
/**
 * redirect to, target for example:
 * 'custframe'、'custframe.subframe'、['custframe',parent]
 */
function redirectTo(page, listener, params, target, israw) {
	...
	if (obj != null) {
		...
		if (subsysaddr != null && !subsysaddr.endsWith("/")) {
			...
			subsysaddr = subsysaddr.substring(0,
					subsysaddr.lastIndexOf("/") + 1);
		}
	}
	if (contextname == null)
...
function redirectToByUrl(url, target, israw, subsysaddr) {
	var obj = getElementBySrc();
	if (israw == null || !israw)
		url = getSysAddr(url, obj == null ? null : obj
				.getAttribute("subsyscode"),
				subsysaddr == null ? (obj == null ? null : obj
						.getAttribute("subsysaddr")) : subsysaddr);
	if (target != null && target != "" && target != "contentframe"
			&& target != "currentframe") {
		...
		var caption = linkobj != null ? linkobj.getAttribute("caption")
				: (obj == null ? null : obj.getAttribute("caption"));
		...
		if (caption != null) {
			if (target != "currentframe") {
				if (addNavFrame(target == null ? parent : target, url, caption))
					return;
			}
		}
		if (linkobj != null) {
...
function redirectToNavByUrl(url, target) {
	if (getFrame(target).location.href == "about:blank") {
		...
		getFrame(target).location.href = getContextName()
				+ "?service=page/component.Navigation&listener=opencontent"
				+ url;
	} else {
...
function clickTabPane(target, page, listener, params, isreload) {
	...
	if (frame.src == null || frame.src == "" || isreload != null && isreload) {
		var parameters = frame.getAttribute("parameters")
				+ (params ? params : "");
...
function hasPriv(privcode) {
	if (pagevisit.getAttribute("staffId") == "SUPERUSR")
		return true;
...
function checkPageSize(obj) {
	if (!checkText(obj, "每页记录数"))
		return false;
	if (!checkNature(obj, "每页记录数"))
		return false;
...
function checkPageChange(obj) {
	...
	if (!checkText(changeInput, "跳转页数"))
		return false;
	if (!checkNature(changeInput, "跳转页数"))
		return false;
	if (parseInt(changeInput.value) > parseInt(changeInput
...
function beforePagination(obj) {
	var param = "&" + obj.getAttribute("currPageName") + "="
			+ obj.getAttribute("currPageValue");
	param += "&" + obj.getAttribute("currPageName") + "_count="
			+ obj.getAttribute("rowCount");
	if (obj.getAttribute("needCount") != null) {
		param += "&" + obj.getAttribute("currPageName") + "_needCount="
				+ obj.getAttribute("needCount");
	}
	if (obj.getAttribute("checkboxName") != null
			&& obj.getAttribute("checkboxName") != "") {
		var checkboxNames = obj.getAttribute("checkboxName").split(",");
		for ( var i = 0; i < checkboxNames.length; i++) {
			var boxname = obj.getAttribute("currPageName") + "_"
					+ checkboxNames[i];
			...
			// param += "&" + obj.getAttribute("currPageName") + "_" +
			// obj.getAttribute("checkboxName") + "=" +
			// getCheckedBoxStr(obj.getAttribute("checkboxName"), "!");
		}
	}
	obj.jsparam = param;
}
...
function afterPagination(paginName, checkboxName) {
	if (checkboxName == null || checkboxName == "" || checkboxName == "null")
		return;
...
function move(source_name, target_name, isall) {
	...
	for ( var i = options.length - 1; i >= 0; i--) {
		...
		if (!containKey(target.options, option.value))
...
function flipMouseDown(obj, name, mode) {
	if (mode == "up") {
		obj.className = "up on";
		if (!flipTrigger(name, "up"))
			return false;
	}
	if (mode == "down") {
		obj.className = "down on";
		if (!flipTrigger(name, "down"))
			return false;
	}
	return true;
}
...
function flipTrigger(name, mode) {
	...
	if (!checkText(field, fieldDesc))
		return false;
	if (!checkInteger(field, fieldDesc))
		return false;
	...
	if (mode == "up") {
		if (maxValue == null
				|| parseInt(fieldValue) + parseInt(degreeValue) <= parseInt(maxValue)) {
			field.value = parseInt(fieldValue) + parseInt(degreeValue);
		}
	}
	if (mode == "down") {
		if (minValue == null
				|| parseInt(fieldValue) - parseInt(degreeValue) >= parseInt(minValue)) {
			field.value = parseInt(fieldValue) - parseInt(degreeValue);
		}
	}
	return true;
...
function getStrByPadLength(value, encode) {
	var length = encode != null && encode == true ? getLength(value)
			: value.length;
	...
	for ( var i = 0; i < theForm.elements.length; i++) {
		...
		if (element.tagName == "INPUT" || element.tagName == "SELECT"
			...
			if (name == null || desc == null || value == null) {
				...
				if (/%|&/.test(value)) {
					alert(element.getAttribute("desc") + "不能输入:" + value);
					focus(element);
					return false;
				}
				if (value == "")
					continue;
				if (element.getAttribute("fieldName") != null
						&& getElement(element.getAttribute("fieldName")) != null) {
					continue;
				}
				isvalid = true;
			...
			if (isvalid) {
				count++;
				str += getStrByPadLength(name) + getStrByPadLength(desc)
						+ getStrByPadLength(value) + getStrByPadLength(text);
			}
		}
	}
...
function encodeCriteriaExport(cond_group) {
	...
	for ( var i = 0; i < document.forms.length; i++) {
		...
		for ( var j = 0; j < theForm.elements.length; j++) {
			...
			var name = element.name;
			var desc = element.getAttribute("desc") == null ? "FIELD_DESC"
			...
			if (element.tagName == "INPUT" || element.tagName == "SELECT"
				...
				if (element.type.toUpperCase() == "HIDDEN") {
					if (element.getAttribute("textName") != null) {
						var popfield = getElement(element
								.getAttribute("textName"));
						if (popfield != null)
							text = popfield.value;
					}
					if (value == "" && text == "")
				...
				if (isvalid) {
					count++;
					str += getStrByPadLength(name) + getStrByPadLength(desc)
							+ getStrByPadLength(value)
							+ getStrByPadLength(text);
				}
			}
		}
	}
...
function configCriteria(obj) {
	var ret = popupDialog(obj.getAttribute("param2"), obj
			.getAttribute("param3"), "&EXEC_CONDITION="
			+ getElementValue("CRIT_REMARK") + obj.getAttribute("parameters"),
			obj.getAttribute("param1"), obj.getAttribute("param4"), obj
					.getAttribute("param5"));
	if (ret == null)
		return;
...
function createCriteria(obj) {
	...
	if (crit_name.value == "" || crit_name.value == "请选择定制方案..."
			|| crit_name.value == "请输入方案名称...") {
		alert("请输入需保存的自定义方案名称！");
		crit_name.focus();
		return false;
	}
	if (!checkMaxLength(crit_name, 200, "方案名称"))
		return false;
	if (!confirmInfo(obj, "确定保存为定制方案么？"))
		return false;

	var params = "&CRIT_CLASS=" + obj.getAttribute("crit_class")
			+ "&CRIT_NAME=" + crit_name.value + "&CRIT_REMARK="
			+ crit_remark.value;
...
function criteriaExport(obj) {
	...
	var query_desc = getElement("QUERY_DESC");
	if (query_desc != null && query_desc.value != '') {
	...
	var hasPrivSimpexp = hasPriv("COM_SIMPLEEXP_"
			+ obj.getAttribute("critClass"));
	var hasPrivCritexp = hasPriv("COM_CRITERIAEXP_"
			+ obj.getAttribute("critClass"));
	...
	if (hasPrivSimpexp && hasPrivCritexp) {
		confirmExport = window
				.confirm("是否执行定时导出？\n按【确定】执行定时导出\n按【取消】可以取消操作或执行即时导出");
	}
	var hasCritexp = confirmExport == null && hasPrivCritexp
			|| confirmExport != null && confirmExport && hasPrivCritexp;
	var hasSimpexp = confirmExport == null && hasPrivSimpexp
			|| confirmExport != null && !confirmExport && hasPrivSimpexp;
	if (hasCritexp) {
		...
		var params = "&CRIT_CLASS=" + obj.getAttribute("critClass")
				+ "&EXEC_PATH=" + obj.getAttribute("exportConfigPath")
				+ "&CONFIG_FILE=" + obj.getAttribute("exportConfigFile")
				+ "&EXEC_CONDITION=" + exec_condition + "&EXEC_GROUP="
				+ obj.getAttribute("conditionGroup") + "&CRIT_DESC="
				+ crit_desc + "&NEED_LOG=" + obj.getAttribute("needLog");
		popupDialog('component.query.ExportConfig', 'inputCriteriaExec',
				params, '定时导出配置', 600, 400);
	}
	if (hasSimpexp) {
		...
		if (!exportFile(exportSimplePath.split("@")[0], exportSimplePath
				.split("@")[1], exec_url, obj.getAttribute("critClass"), null,
				oper_mode))
			return false;
	}
	return true;
...
function exportFile(page, listener, params, privcode, info, oper_mod) {
	...
	try {
		...
		if (oper_mod != null && getVersion() != "BOSS15") {
			redirectTo(page, "createQryLog", "&CRIT_CLASS=" + oper_mod
					+ "&OPER_TYPE=EXP&CRIT_DESC="
					+ getElementValue("QUERY_DESC"), "qryframe");
		}
...
function switchNavFrame(target, navmenuid) {
	...
	if (navmenus.childNodes.length == 0)
		return;
	var navmenu = navmenuid == null ? navmenus.childNodes[0]
			: target.menuframe.document.getElementById(navmenuid);
...
function getNavFrame(target, menuidx) {
	...
	if (navframe != null
			...
			&& target.menuframe.document.getElementById(navframe.name
					+ "_close") == null) {
		return navframe;
	}
	...
	for ( var i = 0; i < target.frames.length; i++) {
		...
		if (!frame.name.startsWith("navframe_"))
			continue;
		if (frame.location.href == "about:blank"
				&& target.menuframe.document.getElementById(frame.name
						+ "_close") == null) {
			return frame;
		}
	}
...
function addNavFrame(target, url, title) {
	...
	if (target.menuframe == null)
		return false;

	//var favoriteMenus = getStorage('favoriteMenus');
	//var favoriteMenus = pagevisit.getAttribute("favoriteMenus");
	//if(!favoriteMenus){
	//	favoriteMenus = pagevisit.getAttribute("favoriteMenus") == null ? "" : pagevisit.getAttribute("favoriteMenus");
	//	setStorage('favoriteMenus',favoriteMenus);
	//}
    // var favoriteMenus = pagevisit.getAttribute("favoriteMenus") == null ? "" : pagevisit.getAttribute("favoriteMenus");
	var rightCode = GetHrefParameter(url,"RIGHT_CODE");
	var favoriteFlag = GetHrefParameter(url,"FAVORITE_FLAG");
	var navmenus = target.menuframe.document.getElementById("navmenus");
	var navmenuidx = target.menuframe.NAV_MENU_CURSOR++;
	var navmenuid = "navmenu_" + navmenuidx;

	//window.localStorage.favoriteMenus = favoriteMenus;
	//setStorage('favoriteMenus',favoriteMenus);
	...
	if (url.indexOf(searchKeyWord) != -1) {
		closeNavMethod = url.substring(url.indexOf(searchKeyWord)
				+ searchKeyWord.length);
		if (closeNavMethod.indexOf("&") != -1)
			closeNavMethod = closeNavMethod.substring(0, closeNavMethod
					.indexOf("&"));
	}
	...
	var navmenucont = "";
	if(favoriteFlag == 1){
		navmenucont = "<a href=\"javascript:void(0)\" class=\"li\" title=\""
				+ title
				+ "\" onclick=\"switchNavFrame(parent, '"
				+ navmenuid
				+ "');\" ondblclick=\"return closeNavFrame(parent, '"
				+ navmenuid
				+ "', '"
				+ closeNavMethod
				+ "');\"></span><span class=\"text\"><span id=\""
				+ navframeid + "_loading\" class=\"loading\">"
				+ getPartStr(title, 20)
				+ "</span></span></a>";
		
		var favmenuClass = "hadcollectTab";
		if(favoriteMenus.indexOf(rightCode) != -1){ 
			favmenuClass = "collectTab"
		}
		
		navmenucont += "<a href=\"javascript:void(0)\" class=\""+favmenuClass+"\" id=\""
		+ navframeid + "_collect\" onclick=\"return doCollectNavMenu(this,'"+ rightCode + "','"+ title + "','"+ url + "');\"/></a>";
	}else{
		navmenucont = "<a href=\"javascript:void(0)\" class=\"li\" title=\""
			+ title
			+ "\" onclick=\"switchNavFrame(parent, '"
			...
			+ "', '"
			+ closeNavMethod
			+ "');\"><span class=\"text2\"><span id=\""
			+ navframeid + "_loading\" class=\"loading\">"
			+ getPartStr(title, 20)
			+ "</span></span></a>";
	}
	
	navmenucont += "<a href=\"javascript:void(0)\" class=\"closeTab\" id=\""
			+ navframeid + "_close\" onclick=\"return closeNavFrame(parent, '"
			+ navmenuid + "', '" + closeNavMethod + "');\"/></a>";
...
function closeNavFrame(target, navmenuid, closeNavMethod) {
	...
	if (target.menuframe.HOLD_FIRST_PAGE
			&& navmenuid.substring("navmenu_".length) == "0") {
		alert("不能关闭主页面！");
		return false;
	}
...
/** collect menu */
function doCollectNavMenu(obj,v_menu_id,title,url){
	    ...
	    //var favoriteMenus = pagevisit.getAttribute("favoriteMenus");
		var favoriteMenus = getStorage('favoriteMenus');
		if(obj.className == "hadcollectTab"){
			...
			if(favoriteMenus.indexOf(v_menu_id) != -1){
				alert("该菜单项已经收藏了，不能再次收藏！");
				return false;
			}
			ajaxDirect("Nav", 'addFavoriteMenus', '&MENU_ID='+v_menu_id+'&STAFF_ID='+staffId+'&EPARCHY_CODE='+epachyId+'&DEPART_ID='+departId, 'FavoriteMenusPart');
			//this.afterAction = "addFavAfterAjax()";
			
			obj.className = "collectTab";
			favoriteMenus = favoriteMenus + "," + v_menu_id;
			setStorage('favoriteMenus',favoriteMenus);
			
			var favul = parent.parent.document.getElementById("favMenus");
			
			var favmenuhtml = "<li class=\"file\">"
				+"<img src=\"component/images/login/useful_a.png\" width=\"7\" height=\"16\">"
				+"<a href=\"javascript:void(0)\" parameters=\"\" title=\""
				+title+"\" "
				+"value=\""+title+"\" onclick=\"clickMenuItem(this);openmenu(getAttribute('menuaddr'));\" "
				+"menuid=\""+v_menu_id+"\" class=\"text\" "
				+"menuaddr=\""+url+"\" style=\"padding-left:5px\">"+ title+"</a> "
				+"<a href=\"javascript:void(0)\" class=\"delTab\" style=\"float: right;\" onclick=\"return doDeleteFavoriteMenu(this);\">"
				+"<img src=\"component/images/login/minus.png\" width=\"16\" height=\"16\">"
				+"</a></li>";
			
			favul.innerHTML = favul.innerHTML + favmenuhtml;
			...
			if(favoriteMenus.indexOf(v_menu_id) == -1){
				alert("该菜单项还未收藏，不能取消收藏！");
				return false;
			}
			ajaxDirect("favo.FavoriteMenu", 'deleteFavoriteMenu', '&MENU_ID='+v_menu_id+'&STAFF_ID='+staffId+'&EPARCHY_CODE='+epachyId+'&DEPART_ID='+departId, '');
			//this.afterAction = "delFavAfterAjax()";
			
			obj.className = "hadcollectTab";
			
		    var num = favoriteMenus.indexOf(v_menu_id);
		    var favoriteMenusbak = "";
		    if(favoriteMenus == v_menu_id ){
		       favoriteMenusbak =  favoriteMenus.replace(v_menu_id,"");
		    }else{
			    if(0 == favoriteMenus.indexOf(v_menu_id)){
			       favoriteMenusbak =  favoriteMenus.replace(v_menu_id+",","");
			    }else{
			       favoriteMenusbak =  favoriteMenus.replace(","+v_menu_id,"");
			    }
		    }
		    setStorage('favoriteMenus',favoriteMenusbak);
		    
			var favlist = parent.parent.document.getElementById("favMenus");
			for(var i = 0; i < favlist.children.length; i++){
				var itemli = favlist.children[i];
				var menu_id = itemli.children[1].getAttribute("menuid");
				if(menu_id == v_menu_id){
					itemli.remove();
				}
			}
...
function addFavAfterAjax(obj,favoriteMenus,v_menu_id,title,url){
	
	var resultFlag = this.ajaxData[0].FLAG;
	obj.className = "collectTab";
	favoriteMenus = favoriteMenus + "," + v_menu_id;
	setStorage('favoriteMenus',favoriteMenus);
...
function delFavAfterAjax(obj,favoriteMenus,v_menu_id){
...
/** del menu */
function doDeleteFavoriteMenu(obj){
	...
	var v_menu_id = $j(obj).prev().attr("menuId");
    ...
    //pagevisit.setAttribute("favoriteMenus",favoriteMenusbak);
    setStorage('favoriteMenus',favoriteMenusbak);
    
    
    //document.getElementsByTagName('meta')['favoriteMenus'] = favoriteMenusbak;
    
    return true;
}

/**
 * hold first nav frame,for example:holdFirstNavFrame("contentframe", true);
 * redirectTo...
 */
...
function getNavFrameByLocation() {
	if (parent.document.getElementById("navframeset") != null) {
		...
		for ( var i = 0; i < frames.length; i++) {
			try {
				framesrc = frames[i].location.href;
			} catch (ex) {
...
function closeNavFrameByLocation() {
	...
	if (navmenu != null) {
		var navframeid = navmenu.getAttribute("relaframe");
		if (navframeid != null) {
			var tabclose = parent.menuframe.document.getElementById(navframeid
					+ "_close")
			if (tabclose != null)
				tabclose.click();
		}
...
function setNavTitle(title) {
	...
	if (navframe != null) {
		var tabload = parent.menuframe.document.getElementById(navframe.name
				+ "_loading");
		...
		if (document.readyState == "complete") {
			if (isSameDomain(parent)) {
				if (parent.document.getElementById("navframeset") != null) {
					var navframe = getNavFrameByLocation();
					if (navframe != null) {
						var tabload = parent.menuframe.document
								.getElementById(navframe.name + "_loading");
						if (tabload != null)
							tabload.className = "";
					}
				} else if (parent.document.getElementById("flowbody") != null) {
					if (self.name != "flowtab" && self.name != "flowsubmit"
							&& self.name != "flowleft"
							&& self.name != "flowright") {
						getFrame([ "flowtab", parent ]).pageflow
								.endFlowLoading();
					}
				} else {
...
function popTroop(param) {
	popupDialog('troop.Troop', 'queryTroops', '&SERIAL_NUMBER='
			+ getElementValue(param), '客户群选择', '600', '400');
}
...
function showGround(obj) {
	if (!obj
			|| !obj.currentStyle
			|| !(obj.currentStyle.position == 'absolute' || obj.currentStyle.position == 'relative'))
		return;
	var ground = obj.background;
	if (!ground) {
		ground = document
				.createElement("<iframe src='about:blank' style='position:absolute; left:0px; top:0px; width:0px; height:0px; display:none' scrolling='no' frameborder='0'></iframe>");
...
function toChineseMoney(money) {
	...
	if (money.indexOf(".") != -1) {
		...
		for ( var i = 0; i < part[1].length; i++) {
			...
			if (i == 1) {
				tmpnewchar = (part[1].charAt(0) == "0" ? "零" : "") + tmpnewchar
						+ "分";
			}
...
function beginPageLoading() {
	...
	if (loading == null) {
		...
		loading.style.display = ""; // added by tangz@2010-4-7 10:50:51

	if (overlay == null) {
		overlay = document.createElement("div");
		overlay.className = "c_overlay";
		overlay.id = "overlay";
		overlay.innerHTML = "<iframe class=\"c_overfrm\" style=\"height:"
				+ getPageHeight() + "px;\" frameborder=no></iframe>";
		document.body.appendChild(overlay);
	} else
		overlay.style.display = ""; // added by tangz@2010-4-7 10:50:51
}
/** end page loading */
function endPageLoading() {
	...
	var overlay = getElement("overlay");
	if (loading != null) {
		loading.style.display = "none"; // document.body.removeChild(loading);
		// modify by tangz@2010-4-7 10:52:04
	}
	if (overlay != null) {
		overlay.style.display = "none"; // document.body.removeChild(overlay);
		// modify by tangz@2010-4-7 10:52:04
	}
}
/** begin page overlay */
function beginPageOverlay() {
	...
	if (pagelay == null) { // added by tangz@2010-4-7 10:50:51
		...
		pagelay.id = "pagelay";
		pagelay.innerHTML = "<iframe class=\"c_overfrm\" style=\"height:"
				+ getPageHeight()
				+ "px;\" frameborder=no scrolling=no></iframe>";
		document.body.appendChild(pagelay);
	} else
		pagelay.style.display = ""; // added by tangz@2010-4-7 10:50:51
}
/** end page overlay */
function endPageOverlay() {
	var pagelay = getElement("pagelay");
	if (pagelay != null) {
		pagelay.style.display = "none"; // document.body.removeChild(pagelay);
		// modify by tangz@2010-4-7 10:52:04
	}
}
/** private end */
...
function beforeDialogClose(jscode) {
	var executecode = 'top.window.opener.' + jscode;
	eval(executecode);
}
/**
 * 设置值
 ...
 * @param textid
 */
function setselectednode(textid, objectid) {
	obj = null;
	if (Browser.isFF) {
		obj = window.returnValue;
		document.getElementById(objectid).value = obj.value;
		document.getElementById(textid).value = obj.text;
	} else {
		obj = window.returnValue;
	}
}
function setattcheUrl(ctlobjid, objectname) {
	var retobj = window.returnValue;
	var ctlobj = getElement(ctlobjid);
	var record = ctlobj.getAttribute("record");
	var reserved = ctlobj.getAttribute("reserved");
	var editobj = getElement(objectname);
	var fileobj = document.createElement("SPAN");
	fileobj.id = retobj.fileId;
	var filehref = record == "true" && reserved == "false" ? "attach?action=delete&file_id="
			+ retobj.fileId
			: "javascript:void(0)";
	fileobj.innerHTML += (ctlobj.innerHTML == "" ? "" : "<br/>")
			+ retobj.fileUrl;
	/*
	 * fileobj.innerHTML += "<a href='" + filehref + "' target='printframe'
	 * onclick='if (!window.confirm(\"确定删除文件么？\")) return
	 * false;hidden(parentElement, true);deleteUploadFile(\"" + objectname +
	 * "\", \"" + retobj.fileId + "\");'><img src='component/images/dellzt.gif'
	 * alt='删除' width='16' height='15' border='0'/></a>";
	 */
	fileobj.innerHTML += '<a href="'
			+ filehref
			+ '" target="printframe" onclick="confirmandDel(this,\''
			+ objectname
			+ '\',\''
			+ retobj.fileId
			+ '\')"><img src="component/images/dellzt.gif" alt="删除" width="16" height="15" border="0"/></a>';
...
function confirmandDel(obj, objectname, fileid) {
	if (!window.confirm("确定删除文件么？"))
		return;
	hidden(obj.parentElement, true);
	deleteUploadFile(objectname, fileid);
