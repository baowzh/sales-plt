var customreport = {
	...
	addBindField : function(nodeInfo, currentCol) {
		...
		/**if(this.isInUseTd(nodeInfo)){
			BootstrapDialog.alert({
				title : '提示',
				draggable : true,
				message : '字段已被占用,选择未绑定字段。'
			});
			return;
		}*/
	...
	_addBindField : function(bindField) {
		this.hideBindField(bindField.colIndexName);
		this.bindfields.push(bindField);
	},
	/**
	 * 覆盖绑定的td
	 */
	hideBindField:function(td_id){
		for (i = 0; i < this.bindfields.length; i++) {
			if (this.bindfields[i].colIndexName == td_id) {
	 ...
	 * 判断td已经占用
	 */
	isInUseTd:function(toAdd){
		for (var i = 0; i < this.bindfields.length; i++) {
			var bindfield = this.bindfields[i];
			if (bindfield.colIndexName == toAdd.colIndexName) {
				return true;
			}
		}
		return false;
